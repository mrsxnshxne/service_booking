...
function serializeBooking(booking: BookingWithClient | (Booking & { client: Client | null })) {
...
bookingsRouter.patch("/:id", gatewayAuth, validate(bookingUpdateSchema), async (req, res) => {
	const id = req.params.id as string
	const booking = (await prisma.booking.findUnique({
		where: { id },
		include: { property: true },
	})) as (BookingWithRelations & { property: { user_id: string } | null }) | null
	if (!booking) throw new DomainError("not_found", "Booking not found")
	if (!booking.property || booking.property.user_id !== req.managerId) {
		throw new DomainError("forbidden", "You do not have access to this booking")
	}
