...
availabilityRouter.get("/", gatewayAuth, async (req, res) => {
	...
	if (property.user_id !== req.managerId) throw new DomainError("forbidden", "You do not have access to this property")
...
availabilityRouter.patch("/", gatewayAuth, validate(availabilityPatchSchema), async (req, res) => {
	const { property_id } = req.params as { property_id: string }
	const property = await prisma.property.findUnique({ where: { id: property_id } })
	if (!property || property.deleted_at !== null) throw new DomainError("not_found", "Property not found")
	if (property.user_id !== req.managerId) throw new DomainError("forbidden", "You do not have access to this property")
...
holdSlotsRouter.delete("/:hold_slot_id", async (req, res) => {
	const { property_id, hold_slot_id } = req.params as { property_id: string; hold_slot_id: string }
