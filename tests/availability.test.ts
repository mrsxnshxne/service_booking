...
describe("computeAvailability", () => {
	...
	test("a blocked period blocks its days with the source, end-exclusive", () => {
		...
		const days = computeAvailability("2026-07-01", "2026-07-04", blocks, [], NOW);
		expect(days[0]).toEqual({ date: "2026-07-01", blocked: false, source: null });
		expect(days[1]).toEqual({ date: "2026-07-02", blocked: true, source: "manual" });
		// end_date is exclusive: the check-out day is free again
		expect(days[2]).toEqual({ date: "2026-07-03", blocked: false, source: null });
	});
	...
	test("an active hold slot blocks its days without a block source", () => {
		...
		expect(days[0]).toEqual({ date: "2026-07-01", blocked: true, source: null });
		expect(days[1]).toEqual({ date: "2026-07-02", blocked: false, source: null });
	});
	...
	test("blocked period source wins over a hold on the same day", () => {
		const blocks: BlockedPeriodInput[] = [
			{ start_date: "2026-07-01", end_date: "2026-07-02", source: "booking_confirmed" },
		];
		const days = computeAvailability(
			"2026-07-01",
			"2026-07-02",
...
describe("isHoldActive", () => {
	test("active until expires_at, expired after", () => {
		...
		expect(isHoldActive(hold("2026-07-01", "2026-07-02", true), NOW)).toBe(false);
	});
});
...
describe("canCreateHold", () => {
	...
	test("allowed on free dates", () => {
		expect(canCreateHold("2026-07-01", "2026-07-05", blocks, [], NOW)).toBe(true);
	});

	test("rejected when overlapping a blocked period", () => {
		expect(canCreateHold("2026-07-12", "2026-07-20", blocks, [], NOW)).toBe(false);
	});

	test("rejected when overlapping an active hold (only one active hold per slot)", () => {
		const holds = [hold("2026-07-03", "2026-07-06")];
		expect(canCreateHold("2026-07-05", "2026-07-08", [], holds, NOW)).toBe(false);
	});

	test("allowed when the conflicting hold has expired", () => {
		const holds = [hold("2026-07-03", "2026-07-06", true)];
		expect(canCreateHold("2026-07-05", "2026-07-08", [], holds, NOW)).toBe(true);
	});

	test("back-to-back with a block is allowed (end-exclusive ranges)", () => {
		expect(canCreateHold("2026-07-05", "2026-07-10", blocks, [], NOW)).toBe(true);
		expect(canCreateHold("2026-07-15", "2026-07-20", blocks, [], NOW)).toBe(true);
	});
});
...
describe("removableManualBlocks", () => {
	const blocks: BlockedPeriodInput[] = [
		...
		{ start_date: "2026-07-03", end_date: "2026-07-08", source: "booking_confirmed" },
		{ start_date: "2026-07-06", end_date: "2026-07-09", source: "ical_booking" },
